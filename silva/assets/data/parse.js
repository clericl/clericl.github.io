// formatting the JSONified file dump using keyword as key
...
fs.readFile('etymwn.json', (err, data) => {

    const entries = JSON.parse(data);
    let arr = [];

    entries.forEach(entry => {
        if (entry.rel === "rel:etymology" || entry.rel === "rel:etymological_origin_of") {
            arr.push(
                {
                    word: entry.lang.split(":")[1].trim(),
                    source: codes[entry.lang.split(":")[0].trim()] || entry.lang.split(":")[0].trim(),
                    rel: entry.rel === "rel:etymology" ? "from" : "to",
                    targetWord: entry.target.split(":")[1].trim(),
                    targetSource: codes[entry.target.split(":")[0].trim()] || entry.target.split(":")[0].trim()
                }
            )
        }
    });

    const jsonObj = JSON.stringify(arr, null, 2);
    fs.writeFileSync('by_etym.json', jsonObj);
})
